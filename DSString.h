...
public:
     ...
     **/
    DSString();
    DSString(const char*); //done
    DSString(const DSString&); //done
    ~DSString();

    /**
     * Overloaded Assignment Operators
     */
    DSString& operator= (const char*) ; //done
    DSString& operator= (const DSString&) ; //done
    ...
    bool operator== (const char*) const ; // done
    bool operator== (const DSString&) const; //done
    bool operator> (const DSString&) const; //done
    bool operator> (const char*);//done
    bool operator< (const char*);//done
    bool operator< (const DSString&) const; //done
     ...
     **/
    DSString substring(int start, int numChars); //done
   ...
   // DSString splitAtDelimeter(const DSString& delimeter);
    //modift this->data to be contents after delimeter and return contents in this->data to be thoose before

//    "abc,def" -> split(',') => this->"def" return "abc"

    DSString toLower();

    int toInt();

    bool contains(DSString &d);
